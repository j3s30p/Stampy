...
description: ESLint·Prettier·tsc 가 뱉는 룰 ID / 에러 코드를 해석하고, 정당한 예외에만 룰 ID 와 근거를 주석으로 남기는 규약.
triggers:
...
- 모든 `@ts-expect-error` 는 **에러 코드 명시 + 한 줄 근거** 필수:

  ```ts
  // @ts-expect-error TS2345 — Repo 인터페이스가 Phase 2 에 readonly 로 변경 예정, 현재 호출부 임시 무시.
  repo.set(key, mutableValue);
  ```

- 에러 코드 없이 `@ts-expect-error` 단독은 PR 리뷰에서 차단.
- `@ts-ignore` 는 **금지** (정적 검사가 다음 에러를 가린다). 항상 `@ts-expect-error` + 코드.

## ESLint suppress 규약

세 가지 조건 모두 만족해야 머지 가능:

1. **룰 ID 명시** — `eslint-disable-next-line` 의 `next-line` 형태 사용. 파일 전체 disable (`/* eslint-disable */`) 은 PR 단위 리뷰 차단.
2. **근거 한 줄** — `--` 구분자 뒤에 한국어 근거.
3. **최소 범위** — next-line 으로 1줄만, 또는 정 필요한 짧은 블록만.

템플릿:

```ts
// eslint-disable-next-line no-restricted-imports -- Mock repository 가 mock fixture 를 가져오는 것은 의도된 격리 패턴 (ADR-004).
import { tourSpots } from '@shared/mocks';
```

## 위반 시 조치 흐름

1. `npm run quality:fast` 실패.
